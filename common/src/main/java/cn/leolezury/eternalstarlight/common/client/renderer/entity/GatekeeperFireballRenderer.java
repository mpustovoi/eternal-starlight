...
public class GatekeeperFireballRenderer extends ThrownItemRenderer<GatekeeperFireball> {
	...
	public void render(GatekeeperFireball entity, float f, float g, PoseStack poseStack, MultiBufferSource multiBufferSource, int i) {
		if (entity.tickCount >= 2 || !(this.entityRenderDispatcher.camera.getEntity().distanceToSqr(entity) < 12.25)) {
			poseStack.pushPose();
			float scale = (Math.min(60f, entity.getSpawnedTicks() + Minecraft.getInstance().getDeltaTracker().getGameTimeDeltaPartialTick(Minecraft.getInstance().level != null && Minecraft.getInstance().level.tickRateManager().runsNormally())) / 60f) * 3f;
