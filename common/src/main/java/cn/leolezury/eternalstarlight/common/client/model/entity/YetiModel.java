		...
		if (entity.getRollState() != 0) {
			switch (entity.getRollState()) {
				case 1 -> {
					...
					float rollAngle = Mth.rotLerp(Minecraft.getInstance().getDeltaTracker().getGameTimeDeltaPartialTick(Minecraft.getInstance().level != null && Minecraft.getInstance().level.tickRateManager().runsNormally()), entity.prevRollAngle, entity.rollAngle);
